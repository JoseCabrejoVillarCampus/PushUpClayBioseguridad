...
appEmpleado.put("/:id", proxyEmpleado, async (req,res)=>{
    ...
    con.query(`UPDATE empleado SET ? WHERE Id = ?`), [jwtData.payload.body, jwtData.payload.params.id],
        (err, result)=>{
            if(err){
...
appEmpleado.delete("/:id", async (req, res)=>{
    ...
    con.query(`DELETE FROM empleado WHERE Id = ?`,jwtData.payload.paramas.id,
        (err, info)=>{
            if(err){
