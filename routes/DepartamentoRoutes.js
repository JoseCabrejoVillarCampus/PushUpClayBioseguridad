...
appDepartamento.put("/:id", proxyDepartamento, async (req,res)=>{
    ...
    con.query(`UPDATE departamento SET ? WHERE Id = ?`), [jwtData.payload.body, jwtData.payload.params.id],
        (err, result)=>{
            if(err){
...
appDepartamento.delete("/:id", async (req, res)=>{
    ...
    con.query(`DELETE FROM departamento WHERE Id = ?`,jwtData.payload.paramas.id,
        (err, info)=>{
            if(err){
