...
const getMunicipioById = (id)=>{
    return new Promise((resolve, reject)=>{
        const sql = [`SELECT * FROM municipio WHERE Id = ?`, id];
...
appMunicipio.put("/:id", proxyMunicipio, async (req,res)=>{
    ...
    con.query(`UPDATE municipio SET ? WHERE Id = ?`), [jwtData.payload.body, jwtData.payload.params.id],
        (err, result)=>{
            if(err){
...
appMunicipio.delete("/:id", async (req, res)=>{
    ...
    con.query(`DELETE FROM municipio WHERE Id = ?`,jwtData.payload.paramas.id,
        (err, info)=>{
            if(err){
